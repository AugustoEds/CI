...
{
    public class ResumoMovtoHelper
    {
        private static ConnectionHelper _connection;

        public ResumoMovtoHelper(ConnectionHelper connection)
        {
        ...
        private string GetSqlFirebird()
        {
            ...
            sb.AppendLine("   and dep.classificacao_dep = 'Laboratório externo'");
        ...
        {
            LogHelper.Log("Gerando dados relatório resumo de movimentação");
            ...
            for (var count = 1; count <= _connection.CountDays; count++)
            {
                ...
                if (dataBase.Day == 11 && dataBase.Month == 4)
                {
                    LogHelper.Process();
                }

                // para garantir que não está sendo duplicado, iremos excluir os registros do dia, se houver
                _connection.SQLServerContext.Database.ExecuteSqlCommand(string.Format("delete from TB_REL_RPT_GARANTIA where day(dt_resumo) = {0} and month(dt_resumo) = {1} and year(dt_resumo) = {2}", dataBase.Day, dataBase.Month, dataBase.Year));
                ...
                foreach (var item in itens)
                {
                    ...
                    if (prod.ID_GRUPO_PRODUTO != config.ID_GRUPO_RPT_GARANTIA && prod.ID_GRUPO_PRODUTO != config.ID_GRUPO_RPA && prod.ID_GRUPO_PRODUTO != config.ID_GRUPO_SUCATA)
                    {
                        continue;
                    }

                    var itemRel = new TB_REL_RPT_GARANTIA();
                    ...
                    _connection.SQLServerContext.TB_REL_RPT_GARANTIA.Add(itemRel);
                }
            }
            ...
            LogHelper.Log("Relatório resumo de movimentação com sucesso");
